...
const Map = () => {
  useEffect(() => {
    ...
    const initializeMap = () => {
      ...
      areas.forEach((area) => {
        const polygon = new window.window.kakao.maps.Polygon({
          map: map,
          path: area.path,
          strokeWeight: 2,
          strokeColor: '#004c80',
        ...
        window.window.kakao.maps.event.addListener(
          ...
          function (mouseEvent: { latLng: any }) {
            ...
            const content = '<div class="area">' + area.name + '</div>';
        ...
        window.window.kakao.maps.event.addListener(
          ...
          function (mouseEvent: { latLng: any }) {
            const content =
              ...
              area.name +
              '</div><div class="size">총 면적 : 약 ' +
              Math.floor(polygon.getArea()) +
              ' m<sup>2</sup></div></div>';
            showInfoWindow(map, mouseEvent.latLng, content);
          },
        );
      });
    };

    const showCustomOverlay = (map: any, position: any, content: string) => {
      customOverlay = new window.window.kakao.maps.CustomOverlay({
        map: map,
        position: position,
        content: content,
        yAnchor: 1,
    ...
    const showInfoWindow = (map: any, position: any, content: string) => {
      const infowindow = new window.window.kakao.maps.InfoWindow({
        removable: true,
        position: position,
        content: content,
      });
    ...
    // API 로드 및 초기화 => kakao api로드 된 후 그려야돼
    if (window.window.kakao && window.window.kakao.maps) {
      window.window.kakao.maps.load(initializeMap);
    } else {
      const script = document.createElement('script');
      script.src =
        '//dapi.window.kakao.com/v2/maps/sdk.js?appkey=%REACT_APP_KAKAO_MAP_KEY%&autoload=false';
